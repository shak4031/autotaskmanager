...
CREATE INDEX IF NOT EXISTS idx_worklog_task  ON work_log(TaskID);
CREATE INDEX IF NOT EXISTS idx_worklog_owner ON work_log(Owner);
"""
...
def update_task_status(task, status, actual_hours=None, action_label=None, tasks_all=None, db_path=None):
    ...
    # If leaving In Progress, close the work_log session and accumulate time
    if prev_status == "In Progress" and status != "In Progress":
        elapsed = 0
        if in_start:
            elapsed = int((now - in_start).total_seconds())
            task["ActualSeconds"] = int(task.get("ActualSeconds", 0)) + max(0, elapsed)
        task["InProgressStart"] = None
        task["ActualHours"] = round(task.get("ActualSeconds", 0) / 3600.0, 2)
        if db_path:
            worklog_end_session(task, db_path, end_ts=now, elapsed_override=elapsed)

    # If entering In Progress, open a new work_log session
    if status == "In Progress" and prev_status != "In Progress":
        if not task.get("InProgressStart"):
            task["InProgressStart"] = now.isoformat()
        if db_path:
            worklog_start_session(task, db_path, start_ts=now)
...
def worklog_start_session(task: dict, db_path: str, start_ts: datetime.datetime | None = None):
    """Start a new work_log session if one isn't already open for this TaskID+Owner."""
    start_ts = start_ts or datetime.datetime.now()
    with db_connect(db_path) as conn:
        # If an open session exists (rare), don't duplicate
        row = conn.execute(
            "SELECT id FROM work_log WHERE TaskID=? AND Owner=? AND end_ts IS NULL ORDER BY id DESC LIMIT 1",
            (task["TaskID"], task["Owner"])
        ).fetchone()
        if row is None:
            conn.execute(
                "INSERT INTO work_log (TaskID, Owner, start_ts) VALUES (?,?,?)",
                (task["TaskID"], task["Owner"], start_ts.isoformat(timespec="seconds"))
            )

def worklog_end_session(task: dict, db_path: str,
                        end_ts: datetime.datetime | None = None,
                        elapsed_override: int | None = None):
    """
    Close the open work_log session for this TaskID+Owner.
    If no open row exists, create a synthetic row using elapsed_override if provided.
    """
    end_ts = end_ts or datetime.datetime.now()
    with db_connect(db_path) as conn:
        row = conn.execute(
            "SELECT id, start_ts FROM work_log WHERE TaskID=? AND Owner=? AND end_ts IS NULL ORDER BY id DESC LIMIT 1",
            (task["TaskID"], task["Owner"])
        ).fetchone()

        if row:
            # Normal close of an open session
            start_dt = datetime.datetime.fromisoformat(row["start_ts"])
            duration = elapsed_override if elapsed_override is not None else int((end_ts - start_dt).total_seconds())
            duration = max(0, int(duration))
            conn.execute(
                "UPDATE work_log SET end_ts=?, duration_sec=? WHERE id=?",
                (end_ts.isoformat(timespec="seconds"), duration, row["id"])
            )
        else:
            # No open session—create a synthetic one if we know elapsed
            if elapsed_override is not None and elapsed_override > 0:
                synthetic_start = end_ts - datetime.timedelta(seconds=int(elapsed_override))
                conn.execute(
                    "INSERT INTO work_log (TaskID, Owner, start_ts, end_ts, duration_sec) VALUES (?,?,?,?,?)",
                    (task["TaskID"], task["Owner"],
                     synthetic_start.isoformat(timespec="seconds"),
                     end_ts.isoformat(timespec="seconds"),
                     int(elapsed_override))
                )
...
def show_kanban_ui(tasks_for_owner, owner, tasks_all, owners, db_path):
    ...
    class ToolTip:
        ...
        def _show(self, event):
            ...
            lbl = tk.Label(self.tip, text=text, justify="left",
    ...
    def make_column(name):
        ...
        tk.Label(col, text=name, bg=KANBAN_COLORS[name], fg=HEADER_COLORS[name], font=header_font, pady=8).pack(fill="x")
        lb = tk.Listbox(col, activestyle='dotbox', selectmode=tk.SINGLE, font=body_font, bd=0, highlightthickness=0)
        # horizontal scrollbar so long labels can be read
        ...
        columns[name] = col; lists[name] = lb; return lb

    for status in KANBAN_STATUSES:
        make_column(status)

    # --- Details / Progress ---
    details = tk.Frame(root, bg="#F9FAFB", bd=1, relief=tk.SOLID); details.pack(fill="x", padx=10, pady=(0,10))
    d_task = tk.Label(details, text="", bg="#F9FAFB", font=("Segoe UI", 11)); d_task.pack(anchor="w", padx=10, pady=(8,2))
    d_pm = tk.Label(details, text="", bg="#F9FAFB", font=("Segoe UI", 10)); d_pm.pack(anchor="w", padx=10)
    d_blocked = tk.Label(details, text="", bg="#F9FAFB", fg="#B91C1C", font=("Segoe UI", 10)); d_blocked.pack(anchor="w", padx=10, pady=(0,8))

    prog = tk.Frame(details, bg="#F9FAFB"); prog.pack(fill="x", padx=10, pady=(0,10))
    proj_lbl = tk.Label(prog, text="Project Progress", bg="#F9FAFB", font=("Segoe UI", 9)); proj_lbl.grid(row=0, column=0, sticky="w")
    proj_pct = tk.Label(prog, text="0%", bg="#F9FAFB", font=("Segoe UI", 9)); proj_pct.grid(row=0, column=2, sticky="e")
    proj_bar = ttk.Progressbar(prog, orient="horizontal", length=640, mode="determinate"); proj_bar.grid(row=0, column=1, padx=8, sticky="we")
    ms_lbl = tk.Label(prog, text="Milestone Progress", bg="#F9FAFB", font=("Segoe UI", 9)); ms_lbl.grid(row=1, column=0, sticky="w", pady=(6,0))
    ms_pct = tk.Label(prog, text="0%", bg="#F9FAFB", font=("Segoe UI", 9)); ms_pct.grid(row=1, column=2, sticky="e", pady=(6,0))
    ms_bar = ttk.Progressbar(prog, orient="horizontal", length=640, mode="determinate"); ms_bar.grid(row=1, column=1, padx=8, sticky="we", pady=(6,0))
    prog.grid_columnconfigure(1, weight=1)

    # --- Buttons (added Audit…) ---
    btns = tk.Frame(details, bg="#F9FAFB"); btns.pack(anchor="w", padx=10, pady=(0,10))
    ...
    audit_btn = ttk.Button(btns, text="Audit…")  # NEW
    for b in (pause_btn, complete_btn, cancel_btn, reopen_btn, reassign_btn, audit_btn):
    ...
    def tooltip_text_for_label(label):
        ...
        if " [" in label and "]" in label:
            try: tid = label.split(" [",1)[1].split("]",1)[0]
            except Exception: pass
        ...
        if t.get("DependsOn"): lines.append(f"Depends on: {', '.join(t['DependsOn'])}")
        return "\n".join(lines)

    # ---------- populate ----------
    def populate_lists():
        for status, lb in lists.items(): lb.delete(0, tk.END)
        for t in tasks_all.values():
            if t.get("Owner") != owner: continue
            text, color = label_for_task(t)
            lb = lists[t["Status"]]
            lb.insert(tk.END, text)
            try: lb.itemconfig(tk.END, foreground=color)
    ...
    def find_task_by_label(label):
        ...
        if not tid and "[" in label and "]" in label:
            tid = label.split("[",1)[1].split("]",1)[0]
        return tasks_all.get(tid) if tid else None
    ...
    def fmt_hms(seconds: int) -> str:
        seconds = max(0, int(seconds)); h = seconds // 3600; m = (seconds % 3600) // 60; s = seconds % 60
        return f"{h:02d}:{m:02d}:{s:02d}"

    def parse_iso(ts):
        try: return datetime.datetime.fromisoformat(ts) if ts else None
        except Exception: return None

    def refresh_details(selection_task=None):
        t = selection_task or get_first_in_progress()
        if not t:
            d_task.config(text="No task selected"); d_pm.config(text=""); d_blocked.config(text="")
            proj_bar["value"] = 0; proj_pct.config(text="0%"); ms_bar["value"] = 0; ms_pct.config(text="0%"); return
        ...
        proj_bar["value"]=p; proj_pct.config(text=f"{p}%"); ms_bar["value"]=m; ms_pct.config(text=f"{m}%")

    def refresh_timer():
        t = get_first_in_progress(); total = 0; session = 0
        if t:
            total = int(t.get("ActualSeconds", 0))
            start = parse_iso(t.get("InProgressStart"))
            if start: session = int((datetime.datetime.now() - start).total_seconds())
    ...
    def move_task_to_status(t, new_status, action_label=None):
        if not ensure_owner(t): return False
        if new_status == "In Progress" and is_blocked(t, tasks_all):
            messagebox.showwarning("Blocked", "This task is blocked by incomplete dependencies."); return False
        if new_status == "In Progress" and not enforce_single_in_progress(t): return False
        update_task_status(t, new_status, action_label=action_label or f"Move -> {new_status}",
                           tasks_all=tasks_all, db_path=db_path)
        populate_lists(); refresh_details(t); return True

    # ---------- DnD ----------
    def on_start_drag(event, status):
        lb = lists[status]; idx = lb.nearest(event.y)
        if idx < 0: return
        label = lb.get(idx); task = find_task_by_label(label)
        if not task: return
        drag_data["task_id"] = task["TaskID"]; drag_data["source"] = status
    ...
    def on_drop(event):
        ...
        target_status = listbox_under_pointer(); source_status = drag_data["source"]
        tid = drag_data["task_id"]; drag_data["task_id"] = None; drag_data["source"] = None
        ...
        if target_status not in allowed.get(t["Status"], set()):
            messagebox.showinfo("Move not allowed", f"Cannot move from {t['Status']} to {target_status}."); return
    ...
    def get_selected_task():
        ...
        for st, lb in lists.items():
            try: idx = lb.curselection()
            except tk.TclError: continue
            if idx:
                label = lb.get(idx[0]); t = find_task_by_label(label)
                if t and t.get("Owner") == owner: return t
        return None
    ...
    def do_pause():
        ...
        if t["Status"] != "In Progress":
            messagebox.showinfo("Pause", "You can only pause an In Progress task."); return
    ...
    def do_complete():
        ...
        if not t: return
        if t["Status"] != "In Progress":
            messagebox.showinfo("Complete", "Start the task before completing it."); return
    ...
    def do_cancel():
        ...
        if t["Status"] not in ("Pending", "In Progress", "Paused"):
            messagebox.showinfo("Cancel", "Only Pending/In Progress/Paused tasks can be canceled."); return
    ...
    def do_reopen():
        ...
        if t["Status"] != "Completed":
            messagebox.showinfo("Reopen", "Only completed tasks can be reopened."); return
    ...
    def do_reassign():
        ...
        win = tk.Toplevel(root); win.title(f"Reassign {t['TaskID']}")
        ...
        combo = ttk.Combobox(win, textvariable=sel, values=display_values, state="readonly", width=30); combo.pack(padx=10, pady=6)
        def ok():
            disp = sel.get()
            ...
            win.destroy()
            populate_lists(); refresh_details()
    ...
    def do_audit():
        t = get_selected_task()
        if not t:
            messagebox.showinfo("Audit", "Select a task first.")
            return

        win = tk.Toplevel(root)
        win.title(f"Audit — {t['TaskID']}")
        win.geometry("800x520+120+120")
        win.grab_set(); win.transient(root)
        container = tk.Frame(win, padx=10, pady=10); container.pack(fill="both", expand=True)

        # Header
        tk.Label(container, text=f"[{t['TaskID']}] {t['Task']}", font=("Segoe UI", 11, "bold")).pack(anchor="w")

        # Split top/bottom: sessions grid + totals, then comment log
        top = tk.Frame(container); top.pack(fill="both", expand=False, pady=(8,6))
        bottom = tk.Frame(container); bottom.pack(fill="both", expand=True)

        # Sessions (Treeview)
        tv = ttk.Treeview(top, columns=("owner","start","end","dur"), show="headings", height=8)
        for c, w in (("owner",120), ("start",180), ("end",180), ("dur",100)):
            tv.heading(c, text=c.capitalize()); tv.column(c, width=w, anchor="w")
        vs = ttk.Scrollbar(top, orient="vertical", command=tv.yview); tv.configure(yscrollcommand=vs.set)
        tv.grid(row=0, column=0, sticky="nsew"); vs.grid(row=0, column=1, sticky="ns")
        top.grid_columnconfigure(0, weight=1)

        # Per-owner totals
        totals_lbl = tk.Label(top, text="Totals: —"); totals_lbl.grid(row=1, column=0, sticky="w", pady=(6,0))

        # Fetch work_log if available
        per_owner = defaultdict(int)
        sessions_loaded = False
        try:
            with db_connect(db_path) as conn:
                rows = conn.execute(
                    "SELECT Owner, start_ts, end_ts, duration_sec FROM work_log WHERE TaskID=? ORDER BY start_ts",
                    (t["TaskID"],)
                ).fetchall()
            for r in rows:
                sessions_loaded = True
                owner_r = r["Owner"]
                start_r = r["start_ts"] or ""
                end_r   = r["end_ts"] or ""
                dur     = int(r["duration_sec"] or 0)
                per_owner[owner_r] += dur
                # pretty duration
                h = dur // 3600; m = (dur % 3600) // 60; s = dur % 60
                tv.insert("", "end", values=(owner_r, start_r, end_r, f"{h:02d}:{m:02d}:{s:02d}"))
        except sqlite3.OperationalError:
            # work_log table missing
            sessions_loaded = False

        if sessions_loaded:
            # Show per-owner totals
            parts = []
            for o, secs in sorted(per_owner.items()):
                h = secs // 3600; m = (secs % 3600) // 60
                parts.append(f"{o}: {h}h {m}m")
            total_secs = sum(per_owner.values())
            H = total_secs // 3600; M = (total_secs % 3600) // 60
            totals_lbl.config(text=f"Totals: {'  |  '.join(parts)}   |   Task Total: {H}h {M}m")
        else:
            # Fallback to task totals if no sessions
            H = int(t.get("ActualSeconds",0)) // 3600
            M = (int(t.get("ActualSeconds",0)) % 3600) // 60
            totals_lbl.config(text=f"Task Total: {H}h {M}m  (no per-session data)")

        # CommentLog viewer
        tk.Label(bottom, text="Comment Log:").pack(anchor="w")
        txt = tk.Text(bottom, height=10, wrap="word")
        txt.pack(fill="both", expand=True)
        if t.get("CommentLog"):
            txt.insert("1.0", str(t["CommentLog"]))
    ...
    audit_btn.configure(command=do_audit)  # NEW

    # Double-click to quick start/pause
    def start_or_pause_selected(_=None):
        t = get_selected_task()
        if not t: return
    ...
    for lb in lists.values():
        lb.bind("<Double-Button-1>", start_or_pause_selected)
